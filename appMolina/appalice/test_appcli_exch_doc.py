...
notes           :
compile and run :
                : cm770@morello-camb-1: $ py test_appcli_exch_doc.py 
                :      -s morello-camb-3.sm.cl.cam.ac.uk 
                :      -p 8282 -f alicedoc_exch_doc.txt
                : cm770@morello-camb-1: $ py test_appcli_exch_doc.py 
                :
...
# I changed this only for the demo on 18 sep 2023
# these scrypts and clases need re-organisation
from appcli_exch_doc import  SSLclientfile 
...
#  parser.add_argument("-s", "--server", help="Server, default is morello-camb-3.sm.cl.cam.ac.uk", default= "morello-camb-3.sm.cl.cam.ac.uk")
  ...
  parser.add_argument("-f", "--file", help="File to send to server, default is alicedoc_encrypted.txt", default= "alicedoc_encrypted.txt")
