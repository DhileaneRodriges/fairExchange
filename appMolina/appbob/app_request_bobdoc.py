"""
title           : app_request_bobdoc.py 
description     : Alice's application (a client) that establishes a secure 
                : chan over ssl with a server if mutual authentication 
                : succeeds.
                :
...
notes           : I have tested only with "localhost" (i.e 127.0.0.1)
                : and port 8282
compile and run : % python3 app_requests_bobdoc.py 
                : alternativey 
                : % test_app_request_bobdoc  
                :
...
CLIENT_CERT_CHAIN = RESOURCE_DIRECTORY / 'bobClient.intermediate.chain.pem'
CLIENT_KEY = RESOURCE_DIRECTORY / 'bobClient.key.pem'
...
class SSLclientfile():
 ...
 def sockconnect(self, ser="localhost", port=8282):
     ...
     self.conn= context.wrap_socket(self.soc, server_hostname="attestable bob CAMB")
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="Bob's app contacys Bob's attestable and requests Alice's doc")
    parser.add_argument("-s", "--server", help="Server, default is localhost", default= "localhost")
    parser.add_argument("-p", "--port", help="Bob's attestable port, default is ", default=8290)
    parser.add_argument("f", "--file", help="File to send to Bob's attestable", default="helloAttestable.txt")
