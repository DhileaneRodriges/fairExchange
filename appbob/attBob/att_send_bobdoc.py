"""
title           : att_send_bobdoc.py
description     : Is a server running in Bob's attestable that upon
                : request by Bob's application, sends Bob's document
                : encrypted under a symmetric key shared between 
                : Alice's attestable and Bob's attestable. 
                :
...
usage           : 
notes           :
compile and run :  cm770@morello-camb-3: $ python3 att_send_bobdoc.py 
                :           -s morello-camb-3.sm.cl.cam.ac.uk 
                :           -f bobdoc_encrypted.txt 
                :
                :  cm770@morello-camb-3: $ python3 att_send_bobdoc.py 
                :
                : Optionally the test_att_send_bobdoc.py testing
...
class SSLserverfile():
    ...
    def __init__(self):
        ...
        """
        print("\nServer running Bob's attestable ... Its PEM pass phrase is: camb\n")
    ...
    def start_server(self, server_Name, port_Number, ser_fileName):
        ...
        print("Sever has been started in Bob's attestable running on host: ", self.server_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--server_name", help="Name of host where Bob's attestable runs", default="localhost")
    parser.add_argument("-p", "--port_number", help="port number used by server", default="8290")
    parser.add_argument("-f", "--file_to_send", help="Name of file to send to Bob's application", default="bobdoc_encrypted.txt")
