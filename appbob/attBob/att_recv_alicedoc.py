...
usage           : 
notes           :
compile and run :  cm770@morello-camb-3: $ python3 att_recv_alicedoc.py 
                :           -s localhost 
...
class SSLserverfile():
    ...
    def __init__(self):
        ...
        """
        print("\nServer running inside Bob's attestable ... Its PEM pass phrase is: camb\n")
    ...
    def start_server(self, server_Name, port_Number, ser_fileName):
        ...
        print("Sever has been started inside Bob's attestable running on host: ", self.server_name)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="Server running in Bob's attestable")
    parser.add_argument("-s", "--server_name", help="Name of host running Bob's attestable", default="localhost")
    parser.add_argument("-p", "--port_number", help="port number used by server", default="8290")
    parser.add_argument("-f", "--file_to_send", help="Name of file to send to client", default="helloServer.txt")
    ...
    print("Host running Bob's attestable is: ", server_name, "port number is:", port_number)
