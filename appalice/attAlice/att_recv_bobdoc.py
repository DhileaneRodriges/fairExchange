...
usage           : 
notes           :
compile and run :  cm770@morello-camb-3: $ python3 att_recv_bobdoc.py 
                :           -s localhost 
                :           -f alicedoc_encrypted.txt 
                :
                :  cm770@morello-camb-3: $ python3 att_recv_bobdoc.py 
                :
                : Optionally the test_att_recv_bobdoc.py testing
...
class SSLserverfile():
    ...
    def __init__(self):
        ...
        """
        print("\nServer running inside Alice's attestable ... Its PEM pass phrase is: camb\n")
    ...
    def start_server(self, server_Name, port_Number, ser_fileName):
        ...
        print("Sever has been started inside Alice's attestable running on host: ", self.server_name)
...
class ClientHandler(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.conn.send(b"Bob's attestable has received Bob's encrypted doc!\n") 
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="Server running in Alice's attestable")
    parser.add_argument("-s", "--server_name", help="Name of host running Alice's attestable", default="localhost")
    parser.add_argument("-p", "--port_number", help="port number used by server", default="8290")
    parser.add_argument("-f", "--file_to_send", help="Name of file to send to client", default="alicedoc_encrypted.txt")
    ...
    print("Host running Alice's attestable is: ", server_name, "port number is:", port_number)
    main(server_name, int(port_number), file_to_send) 
                                        # No file is received by Alice's attestable.
                                        # 11 Sep 2023, I need to update§
